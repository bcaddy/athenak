...
class GOW17Network {
 public:
  ...
  KOKKOS_FUNCTION void evaluate_function(const Real /*t*/, const Real /*dt*/,
    ...
    const auto ghosts = SetupNextStep(y_floor, species_only);
    ...
    // Compute the changes. y_floor, not y_in: the rates were built from the
    // floored state, so using the raw value here would make f the derivative of
    // neither the floored nor the unfloored system.
    for (size_t i = 0; i < neqs - 1; i++) {
      f(i) = rates.creation(i) - y_floor(i) * rates.destruction(i);
    }
